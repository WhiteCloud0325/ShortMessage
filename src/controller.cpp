...
bool Controller::Init(const libconfig::Config &config, const std::string beam_id) {
    ...
    try {
        ...
        if (!common_setting.lookupValue("logic_ip", logic_ip_)) {
            return false;
        }
        if (!common_setting.lookupValue("logic_port", logic_port_)) {
            return false;
        }
    ...
    //thrift client Init
    if (!SendHelper::GetInstance()->Init(logic_ip_, logic_port_)) {
        return false;
    }
...
void Controller::ProcessMessageFromSate() {
    char buf[1024];
    uint16_t len = 0;
    while(!stop_) {
        memset(buf, 0 ,1024);
